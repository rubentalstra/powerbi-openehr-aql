...
**Phase exit criterion:** You can edit M code in VS Code, run `MakePQX pack`, copy the resulting `.pqx` to Power BI Desktop, see the connector load — even if it does nothing yet.
...
- Tech stack table
- Repo layout (tree of `src/`, `tests/`, `docs/`, `dev/`, `.github/`)
...
- `review.md` — runs Claude through `src/` with a "what would you improve" prompt
...
- `OpenEHR.proj` — MSBuild project file (auto-generated by SDK; commit it)
...
**Steps:**
1. In VS Code, open the `powerbi-openehr-aql` folder. Use the Power Query SDK command palette: `Power Query SDK: Create new project`. Name it `OpenEHR`.
2. The SDK generates the file set above into `src/`.
3. Replace the body of `OpenEHR.pq` with the skeleton below.
4. Build: `Power Query SDK: Build project` (or `pwsh dev/scripts/install-connector.ps1` once written).
...
**Files to modify:** `src/OpenEHR.pq`.
...
**Files to modify:** `src/OpenEHR.pq`. Optionally factor into `src/lib/Aql.pqm`.
...
**Files to modify:** `src/OpenEHR.pq` or `src/lib/Schema.pqm`.
...
**Files to modify:** `src/lib/Schema.pqm` (extract from `OpenEHR.pq` if not already).
...
**Files to modify:** `src/lib/Paging.pqm`.
...
**Files to modify:** `src/lib/Navigation.pqm`, `src/OpenEHR.pq`.
...
**Files to modify:** `src/OpenEHR.pq`, `src/lib/Auth.pqm`.
...
**Files to modify:** `src/lib/Auth.pqm`.
...
**Files to modify:** `src/lib/Navigation.pqm`.
...
**Files to modify:** `src/lib/Aql.pqm`.
